...
<div class="container">
    <div class="card bg-dark">
        <header class="card-header bg-dark @if($index === 0) show @else collapsed @endif" id="accordion-header-{{$index}}">
            <button type="button" class="header text-light" style="font-weight:bold; font-size:20px;">Nomor Pesanan : #{{$order->id}}<p class="comment-date">{{ \Carbon\Carbon::parse($order->date)->format('d M Y')}}</p></button>
        ...
        <div  class="card-body @if($index === 0) d-block @else d-none @endif"  id="accordion-body-{{$index}}">
            <h6 class="invoice">Invoice Order : #{{$order->id}}
                <a href="{{route('peddingordersdetil',$order->id)}}" style="text-decoration: underline; margin-left: 55em;">Lihat Detail Pesanan</a>
            </h6>
                ...
                <div class="card-body row">
                    ...
                    <div class="col"> <strong>Status:</strong> <br>
                        ...
                        @elseif($order->status == 'in progress')
                            <span class="badge badge-success" style="font-size: 15px;">in progress</span>
                        @else
                            <span class="badge" style="background-color: rgb(103, 103, 103); font-size:15px;">{{ $order->status }}</span>
                        @endif
                    </div>
            ...
            <a href="{{ route('userprofile') }}" class="btn"style="background-color: green; color: white;" data-abc="true"> <i class="fa fa-chevron-left"></i> Back</a>
            @if($order->status !== 'pending')
    <a href="{{ route('print.struk', $order->id) }}" class="print">
        <img src="{{ asset('assets/img/print.png') }}" style="width: 30px; margin-left:10px;" alt="Print"> <!-- Icon PDF dari direktori public/assets/images -->
    </a>
@endif
        </div>
    </div>
